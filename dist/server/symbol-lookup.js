...
function fileTypePriority(file) {
...
function runRipgrep(repoPath, symbol) {
    ...
    for (const { pattern, globs } of patternGroups) {
        ...
        let output;
        try {
            output = execSync(`rg --json -n ${[...excludeArgs, ...includeArgs].map(a => JSON.stringify(a)).join(' ')} -e ${JSON.stringify(pattern)} -- .`, { cwd: repoPath, encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'] });
        }
        catch (err) {
            const anyErr = err;
...
export function findSymbol(repoPath, symbol) {
